...
Leap.loop({background: true}, {
  ...
  //            console.log("less than");
            ...
            else if(indexPos[0]>150&&indexPos[0]<170&&indexPos[1]>150&&indexPos[1]<170){
              console.log("toggle #output-display-container");
              if(sessionManager.outputTimeOut){
                console.log("wait to press button after timeout");
              }
              else{
                if(sessionManager.outputDisplayContainerVisible){
                  document.getElementById("output-display-container").setAttribute("material", "opacity", 0);
                  sessionManager.outputDisplayContainerVisible = false;
                }
                else{
                  document.getElementById("output-display-container").setAttribute("material", "opacity", 1.0);
                  sessionManager.outputDisplayContainerVisible = true;
                }
                sessionManager.outputTimeOut = true;
                setTimeout(function(){
                  sessionManager.outputTimeOut = false;
                }, 2000);
              }
            }
            else if(indexPos[0]>170&&indexPos[0]<190&&indexPos[1]>210&&indexPos[1]<230){
              console.log("export as LYOKO");
            }
            else if(indexPos[0]>170&&indexPos[0]<190&&indexPos[1]>240&&indexPos[1]<260){
